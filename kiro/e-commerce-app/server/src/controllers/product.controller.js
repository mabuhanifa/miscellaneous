...
class ProductController {
  ...
  getProductsByTags = async (req, res, next) => {
    try {
      ...
      const result = await productService.getProductsByTags(tagsArray, options);
   ...
   */
  getProductsByAttributes = async (req, res, next) => {
    try {
      const {
        page = 1,
        limit = 20,
      ...
      // Filter out pagination and sorting params to get only attributes
      const validAttributes = {};
      for (const [key, value] of Object.entries(attributes)) {
        if (!["page", "limit", "sortBy", "sortOrder"].includes(key)) {
          validAttributes[key] = value.includes(",") ? value.split(",") : value;
        }
      }
      ...
      const result = await productService.getProductsByVariantAttributes(
        validAttributes,
        options
      );
   ...
   */
  getVariantAttributes = async (req, res, next) => {
    try {
      const attributes = await productService.getVariantAttributes();
  ...
  advancedSearch = async (req, res, next) => {
    try {
      ...
      const result = await productService.advancedSearch(filters, options);
